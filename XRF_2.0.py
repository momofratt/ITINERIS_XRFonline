...
for i in range(len(files_list)):  # rinomina standard con prefisso std_
    if not ((files_list[i][1].startswith('T')) & (files_list[i][1][1].isnumeric())):
...
for filter_num in filtri:
    ...
    for i in range(len(files_list)):
        if (files_list[i][1].startswith(filter_num)) & (files_list[i][1][1].isnumeric()) & (time_str_filter in files_list[i][1]):
    ...
    fig.savefig('./results/2fwhm+lod_'  + filenm_prefix.replace('_STD0','') + '_' + filter_num+'_1-39kv.png', format='png', dpi=300)


##############   CALIBRAZIONE

file_39kv     = [f for f in files_list if '4_39kV_' in f[1]]
...
ax.set_yscale('log')
# ax.set_ylim([1,1000])
plt.savefig('./results/'+ filenm_prefix.replace('_STD0','') + '_sensitivity_Rh_anode.png', format='png', dpi=300)
...
for i, fil in zip(range(len(filtri)),filtri):
    ...
    ############### plot PIXE ###################
    pixe_df     = pd.read_csv('./res_PIXE/conc_PIXE_teflon_watman.csv', sep =' ')
    
    # leggi dataframe errori ed appendi sotto dataframe misure
    pixe_err_df = pd.read_csv('./res_PIXE/conc_PIXE_teflon_watman_errors.csv', sep =' ')
    ...
    ax.errorbar(pixe_small.element, pixe_small[fil]/1000, yerr= pixe_small[fil+'_err_abs']/1000, capsize=7, fmt = '_', c='#35b779' , label='SMALL', zorder=2)
    ax.errorbar(pixe_big.element  , pixe_big[fil]/1000  , yerr= pixe_big[fil+'_err_abs']/1000  , capsize=7, fmt = '*', c='#440154' , label='BIG', zorder=2)

    
    ax.set_yscale('log')
    ax.legend(ncol=2)
    fig.suptitle(fil)
    fig.savefig('./results/' + filenm_prefix.replace('_STD0','') + '_conc+mdl+pixe_teflon_what_'+fil+'.png', format='png', dpi=300)
    ...
    ax.scatter(pixe_xrf_frame.element, pixe_xrf_frame['conc[ug/cm^2]']/pixe_xrf_frame[fil] * 1000, label ='conc$_{xrf}$/conc$_{pixe}$')
    ax.grid(which='both')
    ax.set_ylim([0,11])
    ax.legend()
    ...
    ax.set_title(fil)
    # ax.set_yscale('log')
    plt.savefig('./results/' + filenm_prefix.replace('_STD0','')+'_'+fil+'_pixe_xrf_ratio.png', format='png', dpi=300)
    plt.show()
